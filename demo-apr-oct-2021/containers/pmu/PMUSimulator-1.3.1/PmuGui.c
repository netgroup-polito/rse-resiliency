...
/* ---------------------------------------------------------------- */
/*        this function import config file in command line          */
/* ---------------------------------------------------------------- */

int apply_pmu_setup_cli (char * input_file)
{
    /* local variables */
    int  tmp_p, tempi;
    ...
    if (fp1 != NULL)
    {
        ...
        if(read > 0)
        {
            ...
            int ret_val = create_cfg_file(input_file);
        }
        else
        {
            fprintf(stderr, " Not a PMU Setup File. Try again! \n");
            return -1;
        }
    }
    else
    {
        fprintf(stderr, " PMU Setup File doesn't exist! ");
        return -1;
    }
    return 0;
}
