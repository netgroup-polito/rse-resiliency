...
# Set MONO_IOMAP=case to apply Windows case-insensitivity rules to the build
...
if [ $useNightlyBuild -ne 0 ]; then
	wget -O Source.zip ${address}/NightlyBuilds/GridSolutionsFramework/Beta/GridSolutionsFramework.Source.zip >> ${log}
else
...
# Build GSF source
echo Downloading dependencies for Grid Solutions Framework for Mono...
echo
rm -rf gsf/Source/Dependencies/NuGet/*
nuget install packages.config -OutputDirectory gsf/Source/Dependencies/NuGet/
...
echo
cp -rv gsf/Build/Output/Mono/Libraries/* ${service}/Source/Dependencies/GSF  >> ${log}
...
cp -rv ${service}/Source/Applications/${service}/${service}/wwwroot ${destDir} >> ${log}
