...
int main(int argc, char **argv)
{
	...
	{      
		...
		if(argc == 3){
		    // An input file has been passed
		    if(!strncmp(argv[1], "-i", 2)){
		        fprintf(stdout, "Applying input file %s\n", argv[2]);
                int res = apply_pmu_setup_cli(argv[2]);
                while(!res){
                    usleep(10000);
                }
                // There is an error close everything
                kill(pidLocal, SIGINT);
                exit(1);
		    }
		}
